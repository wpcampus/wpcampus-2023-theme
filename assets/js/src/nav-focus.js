...
function initEachNavToggleSubmenu( nav ) {
	// Get the submenus.
	const SUBMENUS = nav.querySelectorAll( '.nav__item ul.nav__item__sub' );
	...
	for ( let i = 0; i < SUBMENUS.length; i++ ) {
		...
		parentMenuItem.querySelector( 'a' ).addEventListener( 'focus', ( e ) => {
			const parentMenuItemsToggled = e.target.parentNode.parentNode.querySelectorAll( 'li.nav-item--toggled-on' );
		...
		SUBMENUS[ i ].parentNode.classList.add( 'nav-item--has-toggle' );
	}
}
...
function toggleSubMenu( parentMenuItem, forceToggle ) {
	...
	let parentMenuItemToggled = parentMenuItem.classList.contains( 'nav-item--toggled-on' );
	...
	if ( parentMenuItemToggled ) {
		...
		parentMenuItem.classList.remove( 'nav-item--toggled-on' );
		subMenu.classList.remove( 'toggle-show' );
		toggleButton.setAttribute( 'aria-label', wp_nav_focus.expand );

		// Make sure all children are closed.
		const subMenuItemsToggled = parentMenuItem.querySelectorAll( '.nav-item--toggled-on' );
		for ( let i = 0; i < subMenuItemsToggled.length; i++ ) {
			toggleSubMenu( subMenuItemsToggled[ i ], false );
		}
	} else {
		// Make sure siblings are closed.
		const parentMenuItemsToggled = parentMenuItem.parentNode.querySelectorAll( 'li.nav-item--toggled-on' );
		for ( let i = 0; i < parentMenuItemsToggled.length; i++ ) {
			toggleSubMenu( parentMenuItemsToggled[ i ], false );
		}

		// Toggle "on" the submenu.
		parentMenuItem.classList.add( 'nav-item--toggled-on' );
		subMenu.classList.add( 'toggle-show' );
		toggleButton.setAttribute( 'aria-label', wp_nav_focus.collapse );
	}
}
...
function getDropdownButton() {
	...
	dropdownButton.setAttribute( 'aria-label', wp_nav_focus.expand );
